...
public class DigraphAlgorithms
{
     ...
     public static int maxVertex(Graph grafo){
         int mayor = 0;
         for(int i = 1; i < grafo.size; ++i){
             if(grafo.getSuccessors(i).size() > grafo.getSuccessors(mayor).size()){
                 mayor = i;
             }
         }
         return mayor;
     ...
     static int [] dijsktra(Graph dg, int source){
         ...
         for (int i=0; i<dist.length; i++) {
             ...
             for (int j=0; j<n.size(); j++) {
                 final int v = n.get(j);
                 final int d = dist[next] + Integer.parseInt(dg.getWeight(next,v));
                 if (dist[v] > d) {
                     dist[v] = d;
                     pred[v] = next;
                 }
     ...
     public static void leerArchivo(String nombreArchivo) throws IOException{
         ...
         int size = 0;
         archivo = br.readLine();
         while(!(archivo = br.readLine()).contains("Arcos")){             
             String[] vertice = archivo.split(" ");
             mapV.put(vertice[0],size); 
             ++size;
         }
         DigraphAL grafo = new DigraphAL(mapV.size());
         while((archivo = br.readLine()) != null){
             String[] line = archivo.split(" ");
             grafo.addArc((int)mapV.get(line[0]), (int)mapV.get(line[1]), line[2]); 
         }
         dibujarGrafo(grafo);
     }
